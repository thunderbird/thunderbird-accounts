...
from thunderbird_accounts.authentication.tasks import (
    get_abandoned_cart_users_for_mailchimp_tag,
...
class GetAbandonedCartUsersForMailchimpTagTestCase(TestCase):
    ...
    def _create_user(self, username_suffix, **kwargs):
        defaults = {
            ...
            'created_at': self.eligible_created_at,
        }
    ...
    def test_includes_eligible_user_without_subscription(self):
        user = self._create_user('abandoned-no-sub')
...
class TagAbandonedCartInMailchimpTaskTestCase(TestCase):
    ...
    def test_skips_user_without_recovery_email(self, mock_add_or_tag):
        ...
        with patch(
            'thunderbird_accounts.authentication.tasks.get_abandoned_cart_users_for_mailchimp_tag'
        ) as mock_get_users:
