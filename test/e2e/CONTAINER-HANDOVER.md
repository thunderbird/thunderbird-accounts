...
- Structure: `tests/` (e.g. `tests/common/sign-up.spec.ts`), `pages/` page-objects incl. **`pages/tb-accts-oidc-page.ts`** (OIDC flows already modeled), `tb-accts-hub-page.ts`, `dashboard-page.ts`, `email-settings-page.ts`; `const/constants.ts`, `const/types.ts`, `const/mocks/paddle.ts`; env templates **`.env.dev.example` / `.env.stage.example` / `.env.prod.example`**.
